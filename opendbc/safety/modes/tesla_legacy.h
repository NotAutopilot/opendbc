...
static int tesla_legacy_fwd_hook(int bus_num, CANPacket_t *to_fwd) {
  ...
  if (bus_num == 0 && tesla_preap) {
    ...
    if (addr == 0x398) { // GTW_carConfig
        ...
        // Original on bus 0 stays on bus 0 (bus_fwd = -1 default)
    }
  ...
  if (bus_num == 2) {
    // 2 -> 0 (PT/Chassis) is standard for AP messages (if any)
    bus_fwd = 0;
    ...
    if (!tesla_external_panda && (addr == 0x488U) && !tesla_legacy_stock_lkas) {
      bus_fwd = -1; // Should we block? Yes, if we replace it.
      // But Pre-AP doesn't have stock LKAS so tesla_legacy_stock_lkas is false.
      // If we send 0x488 on bus 2 (from OP), we want it to go to bus 0.
      // BUT `tesla_legacy_stock_lkas` logic says: if !stock_lkas, block_msg=true (in old code).
      // Wait, old code: `if (!tesla_external_panda && (addr == 0x488U) && !tesla_legacy_stock_lkas) { block_msg = true; }`
      // This means if stock LKAS is NOT active, BLOCK it?
      // No, this logic in old code (line 264 of prev file) was:
      // "Block message if stock LKAS is NOT active" -> This effectively blocks forwarding from 2->0?
      // This seems wrong for OpenPilot control. OpenPilot sends on Bus 0 (or Bus 2 forwarded to 0).
      // If OpenPilot sends on Bus 2 (as per `TESLA_TX_PREAP_MSGS`), it relies on forwarding to Bus 0.
      // If we block it when stock LKAS is inactive, then OP works?
      // Maybe it means "Block *stock* messages from AP" so OP can take over?
      // But Pre-AP has no AP. So nothing to block.
      // If we send on Bus 2, we want it to go to Bus 0.
      // So we should NOT block it if it comes from us (TX hook allows it).
      // FWD hook sees it if it comes from external bus 2 (if we are MITM).
      // For Pre-AP, we are likely just connected to Bus 0 and 2.
      // If we send on Bus 2 internal, it doesn't go through FWD hook?
      // Messages injected by Panda (TX) do NOT go through FWD hook.
      // FWD hook is for messages received on CAN transceivers.
      // So this logic is for blocking *real* AP messages.
      // Since Pre-AP has no real AP, this logic is irrelevant or should be permissive.
      
      // I'll set bus_fwd = 0 (forward everything from 2 to 0) unless specifically blocked.
      // Since there is no AP, forwarding 2->0 just forwards noise or nothing.
      // Safe to leave as is or default to 0.
    }

    // DAS_control
    if ((tesla_external_panda || tesla_hw1) && (addr == das_control_msg) && !tesla_legacy_stock_aeb) {
      bus_fwd = -1; // Block stock DAS control if not AEB
    }
    
    if (tesla_preap && (addr == 0x2b9U)) {
      bus_fwd = -1; // Block DAS control on Pre-AP (from where? it has no AP)
    }
  }
