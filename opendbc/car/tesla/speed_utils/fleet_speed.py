"""Fleet speed adjustment for map-aware cruise control"""
from opendbc.car.tesla.speed_utils.movingaverage import MovingAverage


class FleetSpeed:
    def __init__(self, average_speed_over_x_suggestions: int):
        self.speed_avg = MovingAverage(average_speed_over_x_suggestions)
        self.frame_last_adjustment = 0

    def adjust(self, CS, max_speed_ms: float, frame: int) -> float:
    ...
    @classmethod
    def is_valid(cls, CS, max_speed_ms: float) -> bool:
